...
class SABR_base_model(Vol_model):
    ...
    def __init__(self, beta=1, vov=1, rho=0,                 ATM=None, sigma_0=None, f=None, T_expiry=1,                 vol_type=None, marking_mode='ATM',                 moneyness_lo=-3, moneyness_hi=3, n_strikes=50):
        ...
        delta_bound_down/up : boundaries for implied vol interpolation in terms of moneyness. 
        For example delta_bound_down/up = 1/3, 3 means implied vol will be computed between
        f/3 and f*3.
...
class SABR_MC(SABR_base_model):
    ...
    def MC_generator(self):
        return Ito.SABR(x0=[self.f, self.sigma_0], T=self.T_expiry,                        scheme_steps=self._scheme_steps, keys=['F', 'vol'],                        beta=self.beta, vov=self.vov, rho=self.rho,                        barrier=[0,None], barrier_condition='absorb')
